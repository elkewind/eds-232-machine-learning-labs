...
# This is a fine model but not great
```
...
```{r}
knn_res %>% collect_metrics() # Still using default for k parameter
```
...
fit_knn_cv <- wf_knn_tune %>% 
  ...
  ) # runs 60 different models (5 folds, 12 values of k)
